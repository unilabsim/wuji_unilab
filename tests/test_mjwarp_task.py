...
def test_full_randomization_step_and_partial_reset_isolation(env):
    ...
    palm_pos, palm_quat = state.palm_pose()
    tag_pos, tag_quat = state.tag_pose()
    assert not np.allclose(tag_pos, 0)
    assert not np.allclose(tag_quat, 0)
    np.testing.assert_allclose(tag_pos, state._tag_pos)
    np.testing.assert_allclose(tag_quat, state._tag_quat)
    np.testing.assert_allclose(palm_pos, state._palm_pos)
    np.testing.assert_allclose(palm_quat, state._palm_quat)
